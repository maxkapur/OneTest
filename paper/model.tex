...
\end{algorithm}
The properties of the resultant assignment are well known. $\mu$ is stable. The resultant assignment is strongly student optimal. That is, if another assignment $\mu'$ is chosen from the set of stable assignments, then $\mu(s) \geq_s \mu'(s)$ for all students $s$, and there is at least one student for whom $\mu(s) \geq \mu'(s)$. Similarly, the resultant assignment is strongly school pessimal, meaning any other stable assignment yields the same or better students at each school. Finally, the algorithm is weakly incentive compatible for individual students. That is, no student can obtain a better match than $\mu(s)$ by falsifying her preference list. % Give some citations here.
