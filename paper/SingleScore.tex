...
This article considers a single-score admissions market with multinomial logit student preferences. The market consists of a finite set of schools $C = \{ 1\dots |C| \}$ and continuum of students having unit mass and Lebesgue measure $\eta$.
...
Let us determine a closed-form expression for the demand function $D(\gamma, p)$ for the single-score model with MNL student preferences. First, sort the schools by cutoff, so that
...
\[ p_c^* \in \argmax_{p_c}\bigl\{ u_c(p_c; p_{c'}): p_c \in [0,1] \bigr\}\]
\end{definition}
...
\[ p_c \in [0,1] \cap [p_c^* - \epsilon,  p_c^* + \epsilon] ~\implies~ u_c(p^*_c; p_{c'}) \geq u_c(p_c; p_{c'})  \]
\end{definition}
...
\[\operatorname{BR}_c  (p_{c'} ; u_c) \equiv \argmax_{p_c}\bigl\{ u_c(p_c; p_{c'}): p_c \in [0,1] \bigr\}\]
...
\[u_c = \log D_c(p_c; p_{c'})  + \sigma_c \log p_c \]
...
\Biggr)\Biggr]\]
This point is only valid if $p_{h} \leq \bar p_h \leq p_{h+1}$, but by concavity, if $\bar p_h$ is outside this segment, then the maximum occurs at the nearer of the two endpoints. Hence
\[\hat p_h = \max \Bigl( p_{c-1},  \min \left( p_{c+1}, \bar p_c \right)\Bigr) \]
